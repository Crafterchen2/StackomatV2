...
public abstract class ExpandableBorder extends AbstractBorder {

    //Felder
    private static int globalScale = 3;
    private static final HashSet<ExpandableBorder> updateWatchList = new HashSet<>();
    ...
    //Konstruktoren
    public ExpandableBorder(boolean addToWatchList){
    ...
    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType){
        paintTexture(g,size,texType, BorderDrawSettings.NOT_SET);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale){
        paintTexture(g,size,texType,scale, BorderDrawSettings.NOT_SET);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale, int fillingWidth){
        paintTexture(g,size,texType,scale,fillingWidth, BorderDrawSettings.NOT_SET);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, Color background){
        paintTexture(g,size,texType,background, BorderDrawSettings.NOT_SET);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, BorderDrawSettings... modifier){
        paintTexture(g,size,texType,globalScale,modifier);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale, BorderDrawSettings... modifier){
        paintTexture(g,size,texType,scale,1,modifier);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale, int fillingWidth, BorderDrawSettings... modifier){
        paintTexture(g,size,texType,scale,fillingWidth,new Color(0,0,0,0),modifier);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, Color background, BorderDrawSettings... modifier){
        paintTexture(g,size,texType,globalScale,background,modifier);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale, Color background, BorderDrawSettings... modifier){
        paintTexture(g,size,texType,scale,1,background,modifier);
    }

    public static void paintTexture(Graphics g, Rectangle size, McComponentTextureEnum texType, int scale, int fillWidth, Color background, BorderDrawSettings... modifier){
        ...
        BorderDrawSettings justSetting = BorderDrawSettings.NOT_SET;
        BorderDrawSettings holeSetting = BorderDrawSettings.NOT_SET;
        HashSet<BorderDrawSettings> otherSettings = new HashSet<>();
        for (BorderDrawSettings setting: modifier) {
            switch (setting.getId()){
                case BorderDrawSettings.JUST_ID -> {
                    if (justSetting == BorderDrawSettings.NOT_SET) justSetting = setting;
                }
                case BorderDrawSettings.HOLES_ID -> {
                    if (holeSetting == BorderDrawSettings.NOT_SET) holeSetting = setting;
                }
        ...
        switch (justSetting) {
            case JUST_OUTER -> {otherSettings.add(BorderDrawSettings.NO_INNER);otherSettings.add(BorderDrawSettings.NO_CENTER);otherSettings.add(BorderDrawSettings.NO_FILLING);}
            case JUST_INNER -> {otherSettings.add(BorderDrawSettings.NO_OUTER);otherSettings.add(BorderDrawSettings.NO_CENTER);otherSettings.add(BorderDrawSettings.NO_FILLING);}
            case JUST_CENTER -> {otherSettings.add(BorderDrawSettings.NO_INNER);otherSettings.add(BorderDrawSettings.NO_OUTER);otherSettings.add(BorderDrawSettings.NO_FILLING);}
            case JUST_FILLING -> {otherSettings.add(BorderDrawSettings.NO_INNER);otherSettings.add(BorderDrawSettings.NO_OUTER);otherSettings.add(BorderDrawSettings.NO_CENTER);}
        }
        if (calcCurrentScaleFactor(scale,size.width-(((fillingWidth-1)*2)*scale),size.height-(((fillingWidth-1)*2)*scale)) > texType.getMaxScaleFactor() && !otherSettings.contains(BorderDrawSettings.FORCE_PAINTING)){
            throw new IllegalArgumentException("New scaleFactor '"+calcCurrentScaleFactor(scale,size.width-(((fillingWidth-1)*2)*scale),size.height-(((fillingWidth-1)*2)*scale))+"' is bigger than '"+texType.getMaxScaleFactor()+"', causing visual oddities. Add \"BorderDrawSettings.FORCE_PAINTING\" to modification list to suppress this error.");
        }
        ...
        if (texType.isComplex()){
            ...
            if (holeSetting == BorderDrawSettings.HOLES_FILLING) backgroundColor = fillColor;
            ...
            outerG.dispose();
            drawTexture(g2d, scaleTexture(outerTexture, scale), backgroundColor, size, !otherSettings.contains(BorderDrawSettings.NO_OUTER), !otherSettings.contains(BorderDrawSettings.NO_FILLING));
            ...
            drawTexture(g2d, innerTexture, backgroundColor, new Rectangle(size.x + ((innerCoords.x + fillingWidth-1) * scale), size.y + ((innerCoords.y + fillingWidth-1) * scale), size.width - (((innerCoords.x + fillingWidth-1) * scale)*2), size.height - (((innerCoords.y + fillingWidth-1) * scale)*2)), !otherSettings.contains(BorderDrawSettings.NO_INNER), !otherSettings.contains(BorderDrawSettings.NO_CENTER));
        }   else {
            drawTexture(g2d, texture, backgroundColor, size,!otherSettings.contains(BorderDrawSettings.NO_OUTER),!otherSettings.contains(BorderDrawSettings.NO_CENTER));
        }
    }
    ...
    public static int callUpdatesInWatchList(){
        int updateErrors = 0;
        for (ExpandableBorder expandableBorder : updateWatchList) {
            if(!expandableBorder.globalScaleUpdated()) updateErrors++;
        }
        return updateErrors;
    }
    ...
    //Abstrakte Methoden
    /**
     * Diese Methode sollte bei den entsprechenden ExpandableBorder Instanzen implementiert
    ...
    **/
    ...
    public static HashSet<ExpandableBorder> getUpdateWatchList() {
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
        if (getBorderBackgroundColor() == null) {
            paintTexture(g,new Rectangle(x,y,width,height),getBorderTexType(), BorderDrawSettings.FORCE_PAINTING, BorderDrawSettings.JUST_OUTER, BorderDrawSettings.HOLES_TRANSPARENT);
        } else {
            paintTexture(g,new Rectangle(x,y,width,height),getBorderTexType(),getBorderBackgroundColor(), BorderDrawSettings.FORCE_PAINTING, BorderDrawSettings.JUST_OUTER);
        }
    }
